...
function Lightbox({ images = [], startIndex = 0, open, onClose }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Image viewer" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="relative w-full max-w-[1100px] max-h-[90vh] flex items-center justify-center">
        <button aria-label="Previous image" className="absolute left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 focus:outline-none" onClick={(e) => { e.stopPropagation(); setIndex(i => Math.max(0, i - 1)); }} disabled={index === 0}>
          <PrevIcon className="w-6 h-6 text-white" />
        </button>

        <button aria-label="Next image" className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 focus:outline-none" onClick={(e) => { e.stopPropagation(); setIndex(i => Math.min(images.length - 1, i + 1)); }} disabled={index === images.length - 1}>
          <NextIcon className="w-6 h-6 text-white" />
        </button>

        <div className="w-full h-[80vh] overflow-hidden flex items-center justify-center">
          <img src={images[index]} alt={`Image ${index + 1} of ${images.length}`} className="max-w-full max-h-full object-contain" />
        </div>

        <button ref={closeRef} aria-label="Close image viewer" onClick={(e) => { e.stopPropagation(); onClose(); }} className="absolute top-3 right-3 p-2 rounded-full bg-white/10 hover:bg-white/20 focus:outline-none">
...
/* SampleModal (unchanged) - same implementation you already have (kept here) */
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    api.getProductBySlug(slug)
      .then((p) => {
        ...
        const initialMain = normalized.primary_image || normalized.og_image || normalized.image || metaImages[0] || '/images/placeholder.png';
  ...
  // Auto-open sample modal if navigation state requested it or ?sample=1 is present
  useEffect(() => {
    if (!location) return;
    const qs = new URLSearchParams(location.search || '');
    const want = qs.get('sample') === '1' || location?.state?.openSample === true;
    if (want) {
      // if product is already loaded, open; otherwise wait until product loads
      if (product) setSampleOpen(true);
      else {
        const id = setInterval(() => {
          if (product) { setSampleOpen(true); clearInterval(id); }
        }, 120);
    ...
    return (
      <div className="min-h-screen p-8">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <button onClick={() => navigate('/products')}><Button variant="outline">Back to products</Button></button>
        </div>
      </div>
    );
  }
  ...
  const gallery = [
    ...
    product.og_image,
    product.image,
    ...(Array.isArray(product._metaImages) ? product._metaImages : [])
  ].filter(Boolean);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card className="overflow-hidden">
              <div className="w-full h-[420px] bg-white flex items-center justify-center overflow-hidden cursor-zoom-in" onClick={() => openLightboxAt(0)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') openLightboxAt(0); }}>
                <div onClick={stopClick} role="presentation" className="w-full h-full">
                  <Image src={mainImage || '/images/placeholder.png'} alt={product.title} className="w-full h-full object-contain product-main-image" width={1200} onClick={(e) => { stopClick(e); openLightboxAt(0); }} />
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {gallery.map((g, i) => (
                      <motion.div key={i} whileHover={{ scale: 1.02 }} className="h-20 overflow-hidden rounded-md">
                        <button type="button" onClick={(e) => onThumbnailClick(e, g, i)} onDoubleClick={() => openLightboxAt(i)} className="w-full h-full p-0 bg-transparent border-0 cursor-pointer" aria-label={`View image ${i + 1}`}>
